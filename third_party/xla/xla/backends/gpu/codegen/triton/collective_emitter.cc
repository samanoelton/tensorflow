...
class AllReduceEmitter {
    ...
    switch (emitter.ctx_.strategy) {
      ...
      case AllReduceStrategy::kMultimem:
        return emitter.rewriter_.notifyMatchFailure(
            ...
            "multi-mem all-reduce are not supported yet for "
            "codegeneration.");
    }
  }
 ...
 private:
  ...
  mlir::LogicalResult Initialize() {
    ...
    // For two-shot signal value is always incremented in 2.
  ...
  mlir::Value GetRemoteBufferMemref(mlir::Value rank_idx) {
    ...
    mlir::Value remote_buf_ptr = GetBufferPtr(remote_buf_ptr_base);
  ...
  xtile::TensorValue LoadTileForRank(mlir::Value rank_idx,
    ...
    auto tensor_type =
        mlir::RankedTensorType::get(sub_tile_shape, elem_storage_type_);
    xtile::TensorValue next_tile = xtile::ExtractTileOp::create(
        builder_, tensor_type, remote_buf_memref, offsets, sub_tile_shape,
  ...
  static mlir::Value ExpandAndBroadcastValue(
      mlir::ImplicitLocOpBuilder& builder, mlir::Value value, int dim,
  ...
  // buffer. So for example if the buffer is of size 1024 and the tiles are of
  // size 512 with 2 ranks. For two-shot, each ranks responsibility is 512/2 =
  // 256 per tile. So output shape is always [256].
  //  - Rank0Tile0: [0]; Rank0Tile1: [512]
  //  - Rank1Tile0: [256]; Rank1Tile1: [768]
  // To get the local offset we need to do std::min(0, offset - pid * tile_size)
  ...
  CalculateSubtileOffsetsAndShape(mlir::Value rank_idx) {
    ...
    // See GetBlockLevelFusionConfigForAllReduce for the tiling logic.
  ...
  // And then perform a tensor<T x !ptr<elem_type>> gathered load.
  absl::StatusOr<xtile::TensorValue> GatherLoad() {
    ...
    mlir::Value iota_last_dim = ttir::BroadcastOp::create(
        builder_, mlir::RankedTensorType::get(tile_shape, i32_type), range);
  ...
  mlir::LogicalResult EmitTwoShot() {
    ...
    xtile::TensorValue accumulator =
        LoadTileForRank(world_size_ - 1, self_offsets, self_shape);
    for (int rank = world_size_ - 2; rank >= 0; --rank) {
      xtile::TensorValue next_tile =
    ...
    absl::StatusOr<xtile::TensorValue> gathered_tensor = GatherLoad();
