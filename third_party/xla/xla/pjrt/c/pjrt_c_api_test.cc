...
FieldOffsetsAndSizesForVersion(int major_version, int minor_version) {
  ...
  if (major_version == 0) {
    ...
    if (minor_version >= 95) {
      add_field("PJRT_Client_LoadFromExecutable", kFnPtrSize);
    }
...
TEST_F(PjrtCAbiTestBase, FieldOffsetsAndSizes) {
  absl::flat_hash_map<std::string, std::pair<size_t, size_t>>
      current_api_offsets_and_sizes{
          ...
          {"PJRT_Client_LoadFromExecutable",
           {offsetof(PJRT_Api, PJRT_Client_LoadFromExecutable),
            sizeof(PJRT_Api::PJRT_Client_LoadFromExecutable)}},
      };
