...
absl::StatusOr<std::unique_ptr<PjRtLoadedExecutable>> PjRtCApiClient::Load(
    ...
    const LoadOptions& load_options) {
  if (pjrt_c_api()->PJRT_Client_LoadFromExecutable == nullptr) {
    return absl::UnimplementedError(
        "PJRT_Client_LoadFromExecutable not available in this version of "
        "the PjRT plugin");
  }

  auto c_executable = std::static_pointer_cast<PjRtCApiExecutable>(executable);

  PJRT_Client_LoadFromExecutable_Args load_args;
  load_args.struct_size = PJRT_Client_LoadFromExecutable_Args_STRUCT_SIZE;
  ...
  const PJRT_Api* api = pjrt_c_api();
  RETURN_STATUS_IF_PJRT_ERROR(api->PJRT_Client_LoadFromExecutable(&load_args),
                              api);
